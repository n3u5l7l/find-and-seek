...
function MapSelect({name, imageSource}){
    ...
    return(
        <div className="map-option-box" onDragStart={handleDragStart} role="presentation">
            ...
            <CustomButton to={`/${toLocation}`} whileHover={{scale:1.05, opacity:1}} whileTap={{scale:0.95, opacity:1}} style={{height:"30px"}}>{button}</CustomButton>
        </div>
    )
}
...
export default function Menu ({name}){
  ...
  let screen = ApplyParticle({"mouseTracking": true, "items": items})(AliceCarousel);
  return (
